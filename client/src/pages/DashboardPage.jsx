  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        // Pass month + year to all 3 API calls
        const params = `?month=${month}&year=${year}`;

        const [sumRes, monthRes, txRes] = await Promise.all([
          api.get(`/transactions/summary${params}`),
          api.get("/transactions/monthly"),           // monthly always shows all months
  ...
  return (
    <>
      <style>{`
        ...
        .dash-greeting-sub { font-size: 13px; color: #999; font-weight: 300; margin-bottom: 4px; letter-spacing: 0.03em; text-transform: uppercase; }
        .dash-greeting h1 { font-family: 'Playfair Display', serif; font-size: clamp(26px, 3vw, 36px); color: #0f0f0d; font-weight: 700; margin: 0; }
        .dash-greeting h1 span { background: linear-gradient(90deg, #378ADD, #1d9e75); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
        ...
        .dash-filter-label { font-size: 12px; color: #aaa; font-weight: 400; }
        .dash-select {
          padding: 7px 12px; border: 1.5px solid #e8e8e5;
        ...
        .dash-select:focus { border-color: #378ADD; }
        .dash-reset-btn {
          padding: 7px 14px; border: 1.5px solid #e8e8e5;
          border-radius: 9px; font-size: 12px; font-weight: 500;
          font-family: 'DM Sans', sans-serif;
          color: #888; background: none; cursor: pointer;
          transition: all 0.13s;
        }
        .dash-reset-btn:hover { background: #f4f4f2; color: #333; }
        .dash-filter-active {
        ...
        .dash-cards { display: grid; grid-template-columns: repeat(3, 1fr); gap: 14px; margin-bottom: 28px; }
        .dash-card { border-radius: 16px; padding: 20px 22px; position: relative; overflow: hidden; }
        .dash-card::after { content: attr(data-icon); position: absolute; right: 16px; bottom: 12px; font-size: 36px; opacity: 0.12; pointer-events: none; }
        .dash-card.balance { background: #0f0f0d; color: #fff; }
        .dash-card.income  { background: #f0faf3; border: 1px solid #b8e8c5; }
        .dash-card.expense { background: #fff5f5; border: 1px solid #fcc8c8; }
        .dash-card-label { font-size: 11px; font-weight: 500; text-transform: uppercase; letter-spacing: 0.07em; margin-bottom: 10px; }
        ...
        .dash-card-amount { font-family: 'DM Mono', monospace; font-size: clamp(20px, 2.5vw, 28px); font-weight: 500; line-height: 1; }
        ...
        .dash-card-sub { font-size: 12px; margin-top: 6px; font-weight: 300; }
        .dash-card.balance .dash-card-sub { color: ${summary.balance >= 0 ? "#1d9e75" : "#f87171"}; }
        .dash-card.income  .dash-card-sub { color: #4aac70; }
        .dash-card.expense .dash-card-sub { color: #e06060; }

        .dash-charts { display: grid; grid-template-columns: 1fr 1.8fr; gap: 16px; margin-bottom: 20px; }
        .dash-chart-card { background: #fff; border: 1px solid #ebebea; border-radius: 16px; padding: 20px 20px 12px; }
        .dash-chart-title { font-size: 13px; font-weight: 600; color: #1a1a1a; margin-bottom: 4px; }
        .dash-chart-sub { font-size: 12px; color: #aaa; font-weight: 300; margin-bottom: 18px; }
        .dash-empty { text-align: center; padding: 32px 0 20px; color: #bbb; font-size: 13px; }
        .dash-empty-icon { font-size: 28px; margin-bottom: 8px; }
        .dash-error { background: #fff5f5; border: 1px solid #fcc8c8; border-radius: 12px; padding: 16px 20px; color: #c94040; font-size: 14px; margin-bottom: 24px; }
        .dash-quick { display: flex; align-items: center; justify-content: space-between; background: linear-gradient(135deg, #f0faf3, #e8f4ff); border: 1px solid #d0eaff; border-radius: 14px; padding: 16px 20px; }
        .dash-quick-text { font-size: 14px; color: #333; }
        .dash-quick-text strong { font-weight: 600; color: #0f0f0d; }
        .dash-quick-link { font-size: 13px; font-weight: 500; background: #0f0f0d; color: #fff; padding: 8px 18px; border-radius: 8px; text-decoration: none; transition: background 0.13s; }
      ...
      <div className="dash">
        ...
        <div className="dash-filter-bar">
          <span className="dash-filter-label">Viewing:</span>
          ...
          {!isCurrentMonth && (
            <>
              <span className="dash-filter-active">
                {MONTHS[month - 1]} {year}
              </span>
              <button
        ...
        <div className="dash-cards">
          ...
          <div className="dash-card income" data-icon="💚">
            ...
            <div className="dash-card-sub">{MONTHS[month-1]} {year}</div>
          </div>
        ...
        <div className="dash-charts">
          <div className="dash-chart-card">
            ...
            <div className="dash-chart-sub">{MONTHS[month-1]} {year} expenses</div>
            {pieData.length === 0 ? (
              <div className="dash-empty">
                <div className="dash-empty-icon">🥧</div>
                No expenses in {MONTHS[month-1]}
              </div>
            ) : (
              <ResponsiveContainer width="100%" height={240}>
                <PieChart>
                  <Pie data={pieData} cx="50%" cy="50%" innerRadius={55} outerRadius={88} paddingAngle={3} dataKey="value">
                    {pieData.map((entry, i) => (
                      <Cell key={entry.name} fill={CAT_COLORS[entry.name] || FALLBACK_COLORS[i % FALLBACK_COLORS.length]} />
                    ))}
                  </Pie>
                  <Tooltip content={<PieTooltip />} />
                  <Legend iconType="circle" iconSize={8} formatter={(v) => <span style={{ fontSize: 12, color: "#666" }}>{v}</span>} />
                </PieChart>
              </ResponsiveContainer>
            )}
          </div>
          ...
          <div className="dash-chart-card">
            ...
            <div className="dash-chart-sub">All-time income vs expenses</div>
            {monthly.length === 0 ? (
              <div className="dash-empty">
                <div className="dash-empty-icon">📊</div>
        ...
        <div className="dash-quick">
          <div className="dash-quick-text">
            ...
            <span style={{ fontWeight: 300, fontSize: 13, color: "#666" }}>Add income or expenses and watch your charts update.</span>
          </div>
          <a href="/transactions" className="dash-quick-link">Add transaction →</a>
        </div>
